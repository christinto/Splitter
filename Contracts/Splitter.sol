    ...
    returns (bool success)
{
    ...
    bob.send(balances[msg.sender]);
    //What if Bob comes back and tries to steal Carol's funds before she gets them? 
    //Because balances would still show what Bob is due...
    //Subtract from contract balance -- transfer should throw if it doesn't work
    //so balances of contract wouldn't get affected if attacked...right?
    balances[this] -= balances[msg.sender];
